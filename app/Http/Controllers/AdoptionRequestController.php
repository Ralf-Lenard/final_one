...
use App\Notifications\AdoptionRequestSubmit;
...
class AdoptionRequestController extends Controller
{
    ...
    public function submitAdoptionRequest(Request $request, $id)
    {
        ...
        // Fire the event for the adoption request
        event(new AdoptionRequestSubmitted($adoptionRequest));

        // Notify the admin about the new adoption request
        $admin = User::where('usertype', 'admin')->first();

        // Check if the admin already has 6 notifications and delete the oldest one
        if ($admin && $admin->notifications()->count() >= 6) {
            // Remove the oldest notification
            $admin->notifications()->oldest()->first()->delete();
        }

        // Send notification to the admin
        $admin->notify(new AdoptionRequestSubmit([
            'animalName' => $adoptionRequest->animal_name, // Use the animal's name
            'first_name' => $adoptionRequest->first_name,
            'last_name' => $adoptionRequest->last_name,
            'message' => 'New adoption request by ',
        ]));

        // Log the success message for debugging
        Log::info('Adoption request submitted successfully for animal ID: ' . $animalprofile->id);

        // Redirect back to user home with success message
    ...
    public function approveAdoption($id)
    {
        ...
        $adoptionRequest->approved_by = auth()->id(); // Set the approved_by field
        $adoptionRequest->save();
    ...
    public function rejectAdoption(Request $request, $id)
    {
        ...
        $adoptionRequest->approved_by = auth()->id();
    ...
    public function completeAdoption()
    {
        $completeAdoption = AdoptionRequest::where('status', 'complete')
            ->orderBy('created_at', 'desc')
            ->paginate(1000);

        return view('admin.CompleteAdoption', compact('completeAdoption'));
    }
}
