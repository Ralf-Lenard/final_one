...
class MessagesController extends Controller
{
    ...
    public function send(Request $request)
    {
        ...
        event(new Messages('message sent'));
        // send the response
        return Response::json([
            'status' => '200',
            'error' => $error,
    ...
    public function getContacts(Request $request)
    {
        $users = Message::join('users', function ($join) {
            ...
            ->select('users.id', 'users.name', 'users.email', 'users.avatar', DB::raw('MAX(ch_messages.created_at) as max_created_at'))
            ->groupBy('users.id', 'users.name', 'users.email', 'users.avatar') // Ensure all selected fields are included in GROUP BY
